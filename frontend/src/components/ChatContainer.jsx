                    ...
                    <div className=" mx-auto space-y-1">
                        {messages.map((msg) => (
                            <div key={msg._id} className={`chat ${msg.senderId === authUser._id ? "chat-end" : "chat-start"}`}>
                                <div className={`rounded-xl space-y-2 min-w-24 max-w-2/3 p-2 relative ${msg.senderId === authUser._id ? "bg-cyan-950 rounded-br-none text-white" : "bg-slate-800 rounded-bl-none text-slate-200"}`}>
                                    {msg.image && <img src={msg.image} alt="Shared" className="rounded-md size-48 object-cover hover:object-contain" />}
                                    {msg.text && <p className="text-sm sm:text-base wrap-break-word whitespace-pre-wrap">{msg.text}</p>}
